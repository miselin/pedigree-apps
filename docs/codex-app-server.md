...
and cache scan fix `eff966492`. These also include `/proc/self/exe` support.
...
with SIGSEGV during `config/read`. This SMP failure remains unresolved; the package
is experimental. Authentication, inference, TLS, sandboxed tools, and persistent
storage remain outside the qualification scope.
