...
package=qemu
version=0.12.5
url="http://download.savannah.gnu.org/releases/$package/$package-$version.tar.gz"
...
./configure --cross-prefix=$ARCH_TARGET-pedigree- --extra-cflags="$CFLAGS" \
            --extra-ldflags="$LDFLAGS $LIBS" --static \
            2>&1 > /dev/null
