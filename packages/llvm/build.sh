...
package=llvm
version=2.7
url="http://llvm.org/releases/$version/$package-$version.tgz"
...
CC=$CC CXX=$CXX ../configure --host=$ARCH_TARGET-pedigree --target=$ARCH_TARGET-pedigree \
...
make INSTALLROOT="$OUTPUT_BASE/$package/$version/" install # 2>&1 > /dev/null
